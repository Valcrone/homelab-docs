...
6. Add `README.md`, `docs/PHASE_TEMPLATE.md`, and `.github/workflows/ci.yml` (contents below, or copy from the provided files).
...
- Actions tab shows the CI workflow running on the push (job list may include a Terraform check step that reports "no .tf files yet" — expected at this stage)
- `git tag` lists `v0.1.0-repo-setup`

## Common Mistakes
- Running `gh repo create` before `gh auth login` completes — command will fail with an auth error.
...
- Naming the CI workflow file anything other than a `.yml`/`.yaml` extension inside `.github/workflows/` — GitHub Actions will silently ignore it.
